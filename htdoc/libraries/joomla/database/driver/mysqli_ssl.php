 ...
 * @since  12.1
 */
class JDatabaseDriverMysqli_ssl extends JDatabaseDriver
{
	/**
	...
	public function connect()
	{
		...
		*/
		...
		mysqli_ssl_set($this->connection, '/etc/mysql/certs/client-key.pem','/etc/mysql/certs/client-cert.pem', '/etc/mysql/certs/ca.pem', NULL, NULL);

		mysqli_real_connect($this->connection, $this->options['host'], $this->options['user'], $this->options['password'],$this->options['database'],$this->options['port'],$this->options['socket'],MYSQLI_CLIENT_SSL);

		if($this->connection->errno > 0)
		{
			die('Could not connect to MySQL. Error : '.$this->connection->error);
			exit;
		}
		// Attempt to connect to the server.
		if (!$this->connection || trim($this->connection->error)!="")
		{
			throw new RuntimeException('Could not connect to MySQL.');
		}
