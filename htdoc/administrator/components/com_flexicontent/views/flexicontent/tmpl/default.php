...
<?php if($x>0){echo '<div id="myAlert" class="alert alert-danger">
  <strong>Aviso!</strong> Existem itens pendentes para aprovação.
</div>';};?>

<?php if($y>0){echo '<div id="myAlert" class="alert alert-danger">
  <strong>Aviso!</strong> Existem itens pendentes para revisão.
</div>';};?>
...
<?php endif;?>
		...
		elseif (!$skip_sliders && $config_saved) :
            ...
            if (!isset($ssliders['pending'])):
				$title = JText::_( 'FLEXI_PENDING_SLIDER' ).' - <span class="badge badge-warning">'.$x.'</span>';
			...
			<?php if (!isset($ssliders['revised'])):
				$title = JText::_( 'FLEXI_REVISED_VER_SLIDER' ).' - <span class="badge badge-warning">'.$y.'</span>';
