...
//kunii
function listToken($currentApiId){
  //echo $currentApiId;
  $applicationModels = new ApIPortalModelApplications();
  //$select = '<select id="loja-select">';

  $applications = $applicationModels->getItems();
  //print_r(json_encode ($applications));

  foreach($applications as $application){
  
    $json = json_decode($application);
    if($currentApiId == $application->apis[0]->apiId ){
      $select = $select . '<option value="'.$application->authBase64.'">';
      $select = $select . $application->name;
...
$selectForRedeToken = listToken($currentApiId);
...
//kunii
        var hasRedeToken = true;

        if (!hasRedeToken && !hasApiKey && !hasBasic && !hasOauth && !hasInvoke) {
   ...
   //kunii     
        if (hasRedeToken) {
          var globalRedeToken = $('<fieldset>' +
              '<div class="form-group">' +
              '<label class="col-sm-2 control-label" id="globalRedeTokenLabel" name="globalRedeTokenLabel" for="globalRedeToken"><?= JText::_('COM_APIPORTAL_APITEST_BASE64') ?></label>' +
              '<div class="col-sm-4">' +
              '<select id="globalRedeToken" name="globalRedeToken" class="chzn-done" style="display: none; ">' +
              '<option value="" selected="selected"><?= JText::_('COM_APIPORTAL_APITEST_SELECT_API_KEY') ?></option>' +
              '<?php echo $selectForRedeToken; ?>' +
              '</select>' +
              '</div>' +
              '</div>' +
              '</fieldset>');

          form.append(globalRedeToken);
        }
